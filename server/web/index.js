...
var STACK_BASE = 5461856, STACK_MAX = 218976, DYNAMIC_BASE = 5461856, DYNAMICTOP_PTR = 218800;
...
var ASM_CONSTS = {
 4586: function() {
  ...
  if (jsString == null) jsString = window.prompt("Server: ");
 ...
 5613: function($0) {
  ...
  var p = "/";
  document.cookie = escape(UTF8ToString($0)) + ";path=" + p + ";expires=" + d.toUTCString();
 },
 7026: function($0, $1, $2, $3, $4, $5) {
  ...
  http.onreadystatechange = function() {
   if (this.status == 200) {
    if (this.readyState == this.DONE) {
     var result = Module.ccall("proteo_post_callback", "null", [ "string", "string", "string" ], [ UTF8ToString($1), this.responseText, UTF8ToString($5) ]);
    }
 ...
 7746: function($0, $1, $2, $3, $4) {
  ...
  http.onreadystatechange = function() {
   if (this.status == 200) {
    if (this.readyState == this.DONE) {
     var result = Module.ccall("proteo_get_callback", "null", [ "string", "string", "string" ], [ UTF8ToString($1), this.responseText, UTF8ToString($4) ]);
    }
   } else if (this.status >= 400) {}
  };
 ...
 8685: function($0, $1, $2, $3, $4) {
  var http = new XMLHttpRequest();
  var url = UTF8ToString($0) + "/login";
  var json = JSON.stringify({
   username: UTF8ToString($1),
   timestamp: UTF8ToString($4),
   scriptId: UTF8ToString($2)
  });
  ...
  http.onreadystatechange = function() {
   if (this.status == 200) {
    if (this.readyState == this.DONE) {
     var result = Module.ccall("em_login_function", "number", [ "string" ], [ this.responseText ]);
    } else if (this.readyState == this.HEADERS_RECEIVED) {
     var result = Module.ccall("em_header_callback", "number", [ "string" ], [ this.getAllResponseHeaders() ]);
    }
   } else if (this.status >= 400) {
    console.log("Http status: " + this.status);
    var result = Module.ccall("em_login_function", "number", [ "string" ], [ null ]);
   }
  };
  http.open("POST", url, true);
  http.setRequestHeader("X-HMAC", hashInBase64);
...
var ___tm_current = 218816;

var ___tm_timezone = (stringToUTF8("GMT", 218864, 4), 218864);
...
var _proteo_post_callback = Module["_proteo_post_callback"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["proteo_post_callback"].apply(null, arguments);
};

var _proteo_get_callback = Module["_proteo_get_callback"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["proteo_get_callback"].apply(null, arguments);
};

var _em_login_function = Module["_em_login_function"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["em_login_function"].apply(null, arguments);
};

var _em_header_callback = Module["_em_header_callback"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["em_header_callback"].apply(null, arguments);
};
